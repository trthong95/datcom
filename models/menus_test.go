...
var (
	menuDBTypes = map[string]string{`ID`: `integer`, `OwnerID`: `integer`, `MenuName`: `character varying`, `CreatedAt`: `timestamp with time zone`, `Deadline`: `time without time zone`, `PaymentReminder`: `time without time zone`, `Status`: `integer`}
	_           = bytes.MinRead
)
