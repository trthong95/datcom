...
var (
	orderColumns               = []string{"id", "user_id", "item_id", "created_at", "updated_at"}
	orderColumnsWithoutDefault = []string{"user_id", "item_id", "created_at", "updated_at"}
...
func (orderL) LoadItem(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOrder interface{}, mods queries.Applicator) error {
	var slice []*Order
	var object *Order
	...
	query := NewQuery(qm.From(`items`), qm.WhereIn(`id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Order) SetItem(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Item) error {
	var err error
	if insert {
