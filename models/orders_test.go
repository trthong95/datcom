...
func testOrderToOneItemUsingItem(t *testing.T) {
	...
	if err = local.L.LoadItem(ctx, tx, false, (*[]*Order)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.Item == nil {
...
var (
	orderDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `ItemID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
