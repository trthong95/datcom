...
var (
	userColumns               = []string{"id", "name", "email", "token", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"name", "email", "token", "created_at", "updated_at"}
...
func (userL) LoadOwnerMenus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`menus`), qm.WhereIn(`owner_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.OwnerMenus = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &menuR{}
			}
...
// AddOwnerMenus adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.OwnerMenus.
// Sets related.R.Owner appropriately.
func (o *User) AddOwnerMenus(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Menu) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.OwnerMenus = append(o.R.OwnerMenus, related...)
	}
